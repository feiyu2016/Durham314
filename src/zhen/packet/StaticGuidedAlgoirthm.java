...
//		while(innerTravel_failure == false){
			...
			if(matchedLayoutInfo != null){ // there is a match!
				...
				ArrayList<inputGeneration.ViewNode> widgetList = matchedLayoutInfo.getStayingViewNodes();
				
				for(inputGeneration.ViewNode node: widgetList){ //stayingWidgtsInfo stores the id for views
...
//		this.setUpBreakPoint();
	...
	private Layout matchWithStaticLayout(List<Map<String,String>> currentLayoutInfo){
		...
		for(Layout layout : this.staticLayout){
			ArrayList<inputGeneration.ViewNode> list = layout.getAllViewNodes();
	...
	private String getNextEventForView(Map<String,String> viewInfo, Layout staticLayoutInfo){
		if(enableStaticHandlerInfo){
			...
			ArrayList<inputGeneration.ViewNode> staticList = staticLayoutInfo.getAllViewNodes();
